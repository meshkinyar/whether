...
toMoonPhase x | x `elem` [0, 1]          = Just NewMoon
...
toWeatherCondition isDay x |  inRange  (200, 299) x  = Just Thunderstorm
                           |  inRange  (300, 399) x
...
toCardinalDirection x |  inRange(0, 22)    x 
...
toUVIDescription x | 0    <= x && x < 2.5  = "Low"
